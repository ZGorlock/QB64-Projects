...
'SUB       DownloadGetHeader  (handle%)                  Reads the header of a download
...
'FUNCTION  DownloadTime!      (handle%)                  Returns the time since the start of the download
...
SUB DownloadGetHeader (handle AS INTEGER)
DIM client AS LONG
DIM fh AS LONG
DIM i AS LONG
DIM i2 AS LONG
DIM i3 AS LONG
DIM getresponse AS STRING * DOWNLOAD_GRAB_HEADER
DIM response AS STRING
DIM responsetest AS STRING * DOWNLOAD_GRAB_HEADER
SHARED downloads() AS DOWNLOAD
SHARED downloads_header() AS STRING
IF NOT DownloadValid(handle) THEN EXIT SUB
IF downloads(handle).status <> DOWNLOAD_INPROGRESS THEN EXIT SUB
client = downloads(handle).client
fh = 10000 + handle
GET #client, , getresponse
response = getresponse
IF getresponse = responsetest THEN response = ""
...
IF i THEN
    ...
    IF i2 THEN
        ...
        IF i3 THEN
            ...
            IF LEN(response) > downloads(handle).filesize THEN response = LEFT$(response, downloads(handle).filesize)
            PUT #fh, , response
            downloads(handle).progress = LEN(response)
            ...
            ELSE
                DownloadStop handle
                TIMER(downloads(handle).timer) FREE
                downloads(handle).timer = _FREETIMER
                ON TIMER(downloads(handle).timer, .1) DownloadUpdate handle
                DownloadStart handle
            END IF
        END IF
    END IF
END IF
...
SELECT CASE handle
    ...
    CASE ELSE
        IF NOT DownloadValid(handle) THEN EXIT SUB
        IF downloads(handle).status = DOWNLOAD_COMPLETED THEN EXIT SUB
        TIMER(downloads(handle).timer) OFF
...
DIM client AS LONG
DIM fh AS LONG
DIM getresponse AS STRING * DOWNLOAD_GRAB
DIM response AS STRING
DIM responsetest AS STRING * DOWNLOAD_GRAB
SHARED downloads() AS DOWNLOAD
SHARED downloads_header() AS STRING
SELECT CASE handle
    CASE DOWNLOAD_ALLDOWNLOADS
    ...
    CASE ELSE
        ...
        client = downloads(handle).client
        fh = 10000 + handle
        IF downloads(handle).filesize - downloads(handle).progress < DOWNLOAD_GRAB THEN
            GET #client, , response
        ELSE
            GET #client, , getresponse
            response = getresponse
        ...
        END IF
        PUT #fh, , response
        downloads(handle).progress = downloads(handle).progress + LEN(response)
        IF downloads(handle).progress >= downloads(handle).filesize THEN
            TIMER(downloads(handle).timer) OFF
            CLOSE #fh
            CLOSE client
        ...
        downloads(handle).time = TIMER - downloads(handle).timeinit
        IF downloads(handle).timelimit THEN
            IF downloads(handle).time >= downloads(handle).timelimit THEN
                DownloadStop handle
                IF downloads(handle).quick = DOWNLOAD_QUICK THEN DownloadFree handle
            END IF
        END IF
...
IF file = "" THEN file = filetitle(url)
DO
...
SHARED downloads() AS DOWNLOAD
IF NOT DownloadValid(handle) THEN EXIT SUB
IF downloads(handle).status = DOWNLOAD_COMPLETED THEN downloads(handle).progress = downloads(handle).filesize
...
FUNCTION DownloadTime! (handle AS INTEGER)
SHARED downloads() AS DOWNLOAD
IF NOT DownloadValid(handle) THEN EXIT SUB
DownloadTime = downloads(handle).time
END FUNCTION
