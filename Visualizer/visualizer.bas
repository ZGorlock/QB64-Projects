...
DO
    ...
    IF k$ = CHR$(32) THEN
        ...
        LOOP UNTIL k$ = CHR$(32)
    END IF
    IF k$ = (CHR$(0) + CHR$(59)) THEN GOSUB menu
    ...
    FOR dl = 1 TO num
        ...
        IF l(dl) = 0 THEN
            ...
            FOR dls = 1 TO m(dl)
                ...
                LINE (px1, py1)-(px2, py2), get_color&(r(dl), dl, 128)
            NEXT dls
        END IF
    NEXT dl
    FOR dc = 1 TO num
        CIRCLE (320, 240), r(dc), get_color&(r(dc), dc, 255)
    NEXT dc
    _DISPLAY
LOOP UNTIL k$ = CHR$(27)
SYSTEM
...
DO
    k$ = INKEY$
LOOP UNTIL k$ = ""
IF num <> oldnum OR spd <> oldspd OR colour <> oldcol THEN
    GOSUB setvar
    GOSUB main
END IF
RETURN

FUNCTION get_color& (rad, cx, a)
SELECT CASE colour
    CASE 0
        get_color& = c(cx)
    CASE 1
